...
Public Class AddPickerForm
    Dim manager As New sqlModule.Manager
    Dim conn As New MySqlConnection(Manager.GetConnString)
    Dim sqlquery As String
    Dim sqlcmd As MySqlCommand
    Dim sqlrd As MySqlDataReader
    Public addpickerformcue As Boolean = False
    Private Sub PickerForm_Load(sender As Object, e As EventArgs) Handles Me.Load
        Me.Cursor = Cursors.WaitCursor
        Try
            clearfields()
            callAutoComplete()
            callAutoPopulate()
            txtJobTitle.Text = "Picker"
            getContactNo("Picker", Me)
            txtPickerNo.Text = globalcontactno
            txtFName.Focus()
    ...
    End Sub
    Private Sub AddPickerForm_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Me.Cursor = Cursors.WaitCursor
        Try
        ...
        Try
            ...
            Else
                If MessageBox.Show("Would you like to save the changes in this page?", "Saving", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    ...
                    If CInt(txtPickerNo.Text) <> globalcontactno Then
                        MessageBox.Show("Please take note that the Picker No. will change from " & CInt(txtPickerNo.Text) & " to " & globalcontactno & "." & vbNewLine & "Another user used Picker No. " & CInt(txtPickerNo.Text) & " for its new picker.", "Note:", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        txtPickerNo.Text = globalcontactno
                    End If
                    If myModule.systemerrorfound = False Then
                        MessageBox.Show("Successfully Save", "Save", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        addpickerformcue = legit
                        Me.Close()
                    End If
                End If
            End If
