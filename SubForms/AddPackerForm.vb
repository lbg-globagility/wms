...
Public Class AddPackerForm
    Dim manager As New sqlModule.Manager
    Dim conn As New MySqlConnection(Manager.GetConnString)
    Dim sqlquery As String
    Dim sqlcmd As MySqlCommand
    Dim sqlrd As MySqlDataReader
    Public addpackerformcue As Boolean = False
    Private Sub AddPackerForm_Load(sender As Object, e As EventArgs) Handles Me.Load
        Me.Cursor = Cursors.WaitCursor
        Try
            clearfields()
            callAutoComplete()
            callAutoPopulate()
            txtJobTitle.Text = "Packer"
            getContactNo("Packer", Me)
            txtPackerNo.Text = globalcontactno
            txtFName.Focus()
    ...
    End Sub
    Private Sub AddPackerForm_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Me.Cursor = Cursors.WaitCursor
        Try
        ...
        Try
            ...
            Else
                If MessageBox.Show("Would you like to save the changes in this page?", "Saving", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    ...
                    If CInt(txtPackerNo.Text) <> globalcontactno Then
                        MessageBox.Show("Please take note that the Packer No. will change from " & CInt(txtPackerNo.Text) & " to " & globalcontactno & "." & vbNewLine & "Another user used Packer No. " & CInt(txtPackerNo.Text) & " for its new packer.", "Note:", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        txtPackerNo.Text = globalcontactno
                    End If
                    If myModule.systemerrorfound = False Then
                        MessageBox.Show("Successfully Save", "Save", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        addpackerformcue = legit
                        Me.Close()
                    End If
                End If
            End If
