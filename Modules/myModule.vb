...
#Region "AUTO-COMPLETE"
    ...
    Sub globalautocompleteOrderInfoB(ByVal globalicombobox As ComboBox, ByVal globalformname As Object)
        Try
            Dim orderinfo As New AutoCompleteStringCollection
            Dim cmd1 As New MySqlCommand("SELECT COALESCE(CONCAT(COALESCE(a.companyname,''),' - ', COALESCE(a.accountno,''),' / ',COALESCE(o.ordernumber,''),' (C.O. No.) / ',CONCAT(COALESCE(pl.packinglistno,''),' (Pa.L. No.)')),'') AS 'companyname' FROM packinglist pl LEFT JOIN orders o ON pl.orderid = o.rowid LEFT JOIN accounts a ON o.accountid = a.rowid LEFT JOIN packinglistcartons plc ON pl.rowid = plc.packinglistid WHERE pl.organizationid = " & Z_OrganizationID & " AND plc.`status` = 'Active' AND o.EndDate IS NULL GROUP BY o.rowid ", globalconn)
            Dim cmd2 As New MySqlCommand("SELECT COALESCE(CONCAT(COALESCE(o.ordernumber,''),' (C.O. No.) / ', COALESCE(a.companyname,''),' - ', COALESCE(a.accountno,''),' / ',CONCAT(COALESCE(pl.packinglistno,''),' (Pa.L. No.)')),'') AS 'ordernumber' FROM packinglist pl LEFT JOIN orders o ON pl.orderid = o.rowid LEFT JOIN accounts a ON o.accountid = a.rowid LEFT JOIN packinglistcartons plc ON pl.rowid = plc.packinglistid WHERE pl.organizationid = " & Z_OrganizationID & " AND plc.`status` = 'Active' AND o.EndDate IS NULL GROUP BY o.rowid ", globalconn)
            Dim da1 As New MySqlDataAdapter(cmd1)
            Dim da2 As New MySqlDataAdapter(cmd2)
            Dim ds1 As New DataSet
            Dim ds2 As New DataSet
...
WHERE o.organizationid = {Z_OrganizationID} AND o.status = '{globaliorderstatus}' AND o.ordertype = '{globaliordertype}' AND oi.QtyOrdered > (IFNULL(plo2.`QtyInCarton`, 0) - IFNULL(i.`TotalQtyInCarton`, 0)) GROUP BY o.rowid ORDER BY o.ordernumber) {appendUnionClause(Z_OrganizationID)}"
    ...
    Sub globalautopopulateOrderInfoB(ByVal globalicombobox As ComboBox, ByVal globalformname As Object)
        Try
            globalicombobox.Items.Clear()
            If globalconn.State = ConnectionState.Open Then globalconn.Close()
            Dim sql1 As String = "SELECT COALESCE(CONCAT(COALESCE(o.ordernumber,''),' (C.O. No.) / ', COALESCE(a.companyname,''),' - ', COALESCE(a.accountno,''),' / ',CONCAT(COALESCE(pl.packinglistno,''),' (Pa.L. No.)')),'') AS 'ordernumber' FROM packinglist pl LEFT JOIN orders o ON pl.orderid = o.rowid LEFT JOIN accounts a ON o.accountid = a.rowid LEFT JOIN packinglistcartons plc ON pl.rowid = plc.packinglistid WHERE pl.organizationid = " & Z_OrganizationID & " AND plc.`status` = 'Active' AND o.EndDate IS NULL GROUP BY o.rowid ORDER BY o.ordernumber "
            If globalconn.State = ConnectionState.Closed Then globalconn.Open()
            Dim cmd1 As New MySqlCommand(sql1, globalconn)
            Dim reader1 As MySqlDataReader = cmd1.ExecuteReader()
            While reader1.Read()
