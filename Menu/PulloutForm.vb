...
#Region "Page Setup"
    ...
    Sub getCountPageNum()
        Try
            ...
            dtCid = getDataTableForSQL("SELECT COUNT(po.rowid) FROM orders po WHERE po.organizationid = " & Z_OrganizationID & " AND po.ordertype = 'Pull-Out' ")
    ...
    Sub getCountPageNum1(ByVal esearchstring As String)
        Try
            ...
            If conn.State = ConnectionState.Open Then conn.Close()
            Dim dtCid As New DataTable
            dtCid = getDataTableForSQL("SELECT COALESCE(COUNT(po.rowid),0) FROM orders po LEFT JOIN accounts su ON po.accountid = su.rowid WHERE po.organizationid = " & Z_OrganizationID & " AND po.ordertype = 'Pull-Out' " & _
    ...
    Sub getCountPageNum2(ByVal edatesearch As String)
        Try
            ...
            If conn.State = ConnectionState.Open Then conn.Close()
            Dim dtCid As New DataTable
            dtCid = getDataTableForSQL("SELECT COALESCE(COUNT(po.rowid),0) FROM orders po WHERE po.organizationid = " & Z_OrganizationID & " AND po.ordertype = 'Pull-Out' AND " & _
    ...
    Sub getCountPageNum3(ByVal ecommonstring As String, ByVal edatesearch As String)
        Try
            ...
            If conn.State = ConnectionState.Open Then conn.Close()
            Dim dtCid As New DataTable
            dtCid = getDataTableForSQL("SELECT COALESCE(COUNT(po.rowid),0) FROM orders po WHERE po.organizationid = " & Z_OrganizationID & " AND po.ordertype = 'Pull-Out' AND " & ecommonstring & " " & edatesearch & " ")
            If dtCid.Rows.Count <> 0 Then
                countpagenum = dtCid.Rows(0)(0)
            Else
                countpagenum = 0
...
#Region "AutoComplete"
    Sub autocompleteCustomerName(ByVal icombobox As ComboBox)
        Try
            ...
            Dim cmd As New MySqlCommand("SELECT COALESCE(CONCAT(COALESCE(su.companyname,''),' - ',COALESCE(su.accountno,'')),'') AS 'customername' FROM orders po LEFT JOIN accounts su ON po.accountid = su.rowid WHERE po.organizationid = " & Z_OrganizationID & " AND po.ordertype = 'Pull-Out' GROUP BY su.accountno ORDER BY su.accountno ", conn)
    ...
    Sub autocompleteStatus(ByVal icombobox As ComboBox)
        Try
            ...
            Dim cmd As New MySqlCommand("SELECT COALESCE(po.status,'') AS 'postatus' FROM orders po WHERE po.organizationid = " & Z_OrganizationID & " AND po.ordertype = 'Pull-Out' GROUP BY po.status ORDER BY po.status ", conn)
            Dim ds As New DataSet
            Dim da As New MySqlDataAdapter(cmd)
            da.Fill(ds, "list")
            Dim i As Integer
...
#Region "AutoPopulate"
    ...
    Sub autopopulateCustomerName(ByVal icombobox As ComboBox)
        Try
            ...
            If conn.State = ConnectionState.Open Then conn.Close()
            Dim sql1 As String = "SELECT COALESCE(CONCAT(COALESCE(su.companyname,''),' - ',COALESCE(su.accountno,'')),'') AS 'Customername' FROM orders po LEFT JOIN accounts su ON po.accountid = su.rowid WHERE po.organizationid = " & Z_OrganizationID & " AND po.ordertype = 'Pull-Out' GROUP BY su.accountno ORDER BY su.companyname "
    ...
    Sub autopopulateStatus(ByVal icombobox As ComboBox)
        Try
            icombobox.Items.Clear()
            If conn.State = ConnectionState.Open Then conn.Close()
            Dim sql1 As String = "SELECT COALESCE(po.status,'') AS 'postatus' FROM orders po WHERE po.organizationid = " & Z_OrganizationID & " AND po.ordertype = 'Pull-Out' GROUP BY po.status ORDER BY po.status "
            If conn.State = ConnectionState.Closed Then conn.Open()
            Dim cmd1 As New MySqlCommand(sql1, conn)
            Dim reader1 As MySqlDataReader = cmd1.ExecuteReader()
            While reader1.Read()
...
#Region "Datagrids"
    Sub displayPullOutList(ByVal istartpage As Integer)
        Try
            ...
            Dim sql1 As String = "SELECT po.rowid,COALESCE(po.ordernumber,''),DATE_FORMAT(po.orderdate,'%d-%b-%Y'),COALESCE(CONCAT(COALESCE(su.companyname,''),' - ',COALESCE(su.accountno,'')),'')," & _
                        "COALESCE(po.status,'') FROM orders po LEFT JOIN accounts su ON po.accountid = su.rowid WHERE po.organizationid = " & Z_OrganizationID & " AND po.ordertype = 'Pull-Out' " & _
    ...
    Sub displaySearchPhrase(ByVal isearchphrase As String, ByVal istartpage As Integer)
        Try
            ...
            If conn.State = ConnectionState.Closed Then conn.Open()
            Dim sql1 As String = "SELECT po.rowid,COALESCE(po.ordernumber,''),DATE_FORMAT(po.orderdate,'%d-%b-%Y'),COALESCE(CONCAT(COALESCE(su.companyname,''),' - ',COALESCE(su.accountno,'')),'')," & _
                        "COALESCE(po.status,'') FROM orders po LEFT JOIN accounts su ON po.accountid = su.rowid WHERE po.organizationid = " & Z_OrganizationID & " AND po.ordertype = 'Pull-Out' AND " & _
...
#End Region
#Region "Printing"
    Sub printPullOut(ByVal iorderid As Integer)
        Try
            If conn.State = ConnectionState.Closed Then conn.Open()
            Dim sql1 As String = "SELECT o.rowid,COALESCE(o.ordernumber,''),DATE_FORMAT(o.orderdate,'%d-%b-%Y'),COALESCE(CONCAT(COALESCE(su.companyname,''),' - ',COALESCE(su.accountno,'')),''),COALESCE(p.productcode,'')," & _
                        "COALESCE(c.colorname,''),COALESCE(pcs.size,''),COALESCE(pcs.seasoncode,''),COALESCE(pcs.sku,''),COALESCE(ci.unitofmeasure,''),COALESCE(ci.qtyordered,0),COALESCE(ci.qtyreceived,0) FROM orderitems ci " & _
                        ...
                        "LEFT JOIN orders o ON ci.orderid = o.rowid LEFT JOIN accounts su ON o.accountid = su.rowid WHERE ci.orderid = " & iorderid & " AND ci.organizationid = " & Z_OrganizationID & " AND ci.`status` != 'Inactive' AND (ci.itemtype != 'BI' AND ci.itemtype != 'A') ORDER BY p.productcode,c.colorname,pcs.size "
            Dim cmd1 As New MySqlCommand(sql1, conn)
            Dim reader1 As MySqlDataReader = cmd1.ExecuteReader
            Dim seqno As Integer = 1
            While reader1.Read()
                If reader1.HasRows Then
                    printdataset.AddSetDRow("Pull-Out No.: " & CStr(reader1(1)) & "", "Pull-Out Date: " & CStr(reader1(2)) & "", "", "", "Customer Name: " & CStr(reader1(3)) & "", "", Format(seqno, "#,##0"), CStr(reader1(4)), CStr(reader1(5)), CStr(reader1(6)), CStr(reader1(7)), CStr(reader1(8)), CStr(reader1(9)), Format(CInt(reader1(10)), "#,##0"), "", "", "", "", "", "", "")
                    seqno = seqno + 1
                End If
            End While
            reader1.Close()
...
#End Region
        ...
        Try
            ...
            If cue = "New" Then
                If LTrim(txtPullOutNo.Text) <> "" Then
                    getOrderIDSupB(txtPullOutNo.Text, "Pull-Out", Me)
            ...
            ElseIf cue = "Edit" Then
                If dgPullOutList.Rows.Count <> 0 Then
                    If LTrim(txtPullOutNo.Text) <> "" Then
                        getOrderIDSupA(CInt(dgPullOutList.CurrentRow.Cells("so_rowid").Value), txtPullOutNo.Text, "Pull-Out", Me)
                        poorderid = globalorderid
                        If poorderid <> 0 Then
                            errProvider.SetError(txtPullOutNo, "Pull-Out No. has been created already, please type a new one.")
                        End If
        ...
        Try
            ...
            If dgPullOutList.Rows.Count <> 0 Then
                ...
                If globalorderstatus <> txtStatus.Text Then
                    MessageBox.Show("This Pull-Out no. has been updated by other user, please click refresh button to check the new status of this order.", "Update", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Exit Try
        ...
        Try
            ...
            ElseIf cue = "Edit" Then
                ...
                If dgPullOutList.Rows.Count <> 0 Then
                    getOrderStatus(CInt(dgPullOutList.CurrentRow.Cells("so_rowid").Value), Me)
                    If globalorderstatus <> txtStatus.Text Then
                        MessageBox.Show("This Pull-Out has been updated by other user, please click refresh button to check the new status of this order.", "Update", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        Exit Try
                    End If
                    If LTrim(cboCustomerName.Text) <> "" Then
                        getCustomerID(cboCustomerName.Text, Me)
                        ...
                        If LTrim(txtPullOutNo.Text) <> "" Then
                            ...
                            End If
                        Else
                            errProvider.SetError(pbAddCustomer, "Please enter the customer Pull-Out no.")
                            Exit Try
                        End If
                    Else
                        errProvider.SetError(pbAddCustomer, "Please enter the customer name.")
                    ...
                    End If
                Else
                    errProvider.SetError(pbAddCustomer, "System cannot find the pull-out to be updated.")
                    Exit Try
                End If
            End If
            myModule.systemerrorfound = False
    ...
    '            getOrderStatus(CInt(dgPullOutList.CurrentRow.Cells("so_rowid").Value), Me)
    '            If globalorderstatus <> txtStatus.Text Then
    '                MessageBox.Show("This pull-out has been updated by other user, please click refresh button to check the new status of this pull-out.", "Update", MessageBoxButtons.OK, MessageBoxIcon.Information)
    '                Exit Try
    '            End If
    '            If MessageBox.Show("Would you like to print this pull-out?", "Printing", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
    '                Me.Cursor = Cursors.WaitCursor
    '                getOrderStatus(CInt(dgPullOutList.CurrentRow.Cells("so_rowid").Value), Me)
    '                If globalorderstatus <> txtStatus.Text Then
    '                    MessageBox.Show("This pull-out has been updated by other user, please click refresh button to check the new status of this pull-out.", "Update", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    '                End If
    '                If myModule.systemerrorfound = False Then
    '                    printPullOut(CInt(dgPullOutList.CurrentRow.Cells("so_rowid").Value))
    '                    Dim printreport As New PullOutPrint
    ...
    '            End If
    '        Else
    '            errProvider.SetError(txtPullOutNo, "System cannot find the Pull-Out to be printed.")
    '            Exit Try
    '        End If
    '    Catch ex As Exception
    '        MsgBox(getErrExcptn(ex, Me.Name))
    ...
    '    Me.Cursor = Cursors.Default
    'End Sub
    Private Sub tsPrintA_Click(sender As Object, e As EventArgs) Handles tsPrintA.Click
        Me.Cursor = Cursors.WaitCursor
        Try
            errProvider.Clear()
            myModule.systemerrorfound = False
            ...
            If dgPullOutList.Rows.Count <> 0 Then
                ...
                If MessageBox.Show("Would you like to print this pull-out?", "Printing", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    ...
                    If myModule.systemerrorfound = False Then
                        printPullOut(CInt(dgPullOutList.CurrentRow.Cells("so_rowid").Value))
                        Dim printreport As New PullOutAPrint
                        Dim openreportviewer As New ReportViewer
                        openreportviewer.CrystalReportViewer.ReportSource = printreport
                        printdatatable = printdataset
                        printreport.SetDataSource(printdatatable)
        ...
        Me.Cursor = Cursors.Default
    End Sub
    Private Sub tsPrintB_Click(sender As Object, e As EventArgs) Handles tsPrintB.Click
        Me.Cursor = Cursors.WaitCursor
        Try
            errProvider.Clear()
            myModule.systemerrorfound = False
            ...
            If dgPullOutList.Rows.Count <> 0 Then
                ...
                If globalorderstatus <> txtStatus.Text Then
                    ...
                    Exit Try
                End If
                Dim addpulloutdetailslinkform As New AddPullOutDetailsForm
                addpulloutdetailslinkform.apodorderid = CInt(dgPullOutList.CurrentRow.Cells("so_rowid").Value)
                addpulloutdetailslinkform.ShowInTaskbar = False
                addpulloutdetailslinkform.ShowDialog()
            Else
                errProvider.SetError(txtPullOutNo, "System cannot find the Pull-Out to be printed.")
                Exit Try
            End If
