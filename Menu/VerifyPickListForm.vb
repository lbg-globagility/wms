    ...
    Private Async Function displayPickListItemsA(ByVal ipicklistid As Integer) As Task
        Try
            ...
            Dim sql1 As String = "SELECT COALESCE(oi.productcolorsizeid,0),COALESCE(c.colorvalue,''),COALESCE(p.productcode,''),COALESCE(c.colorname,''),COALESCE(pcs.size,''),COALESCE(pcs.seasoncode,''),COALESCE(pcs.sku,''),COALESCE(oi.unitofmeasure,''),COALESCE(plo.status,'')," &
                    ...
                    "LEFT JOIN products p ON pc.productid = p.rowid LEFT JOIN orders o ON o.RowID=oi.OrderID LEFT JOIN productinventorylocation pil ON pil.RowID=oi.ProductInventoryLocationId LEFT JOIN rackshelfcolumn r ON r.RowID=pil.RackShelfColumnID LEFT  JOIN inventorylocations il  ON il.RowID=r.InventoryLocationID WHERE plo.organizationid = " & Z_OrganizationID & " AND (plo.`status` != 'Inactive' AND plo.`status` != 'Cancelled') " &
...
#End Region
    ...
    Private Sub linkManageVerification_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles linkManageVerification.LinkClicked

        If dgPickList.CurrentRow Is Nothing Then Return

        Dim form = New PickListVerficationForm(CInt(dgPickList.CurrentRow.Cells("pl_rowid").Value))

        If Not form.ShowDialog() = DialogResult.OK Then Return

        cmdFirst_Click(sender:=cmdFirst, e:=New EventArgs())
